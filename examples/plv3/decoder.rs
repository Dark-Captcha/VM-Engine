...
use vm_engine::ir::operand::{Operand, SourceLoc};
use vm_engine::ir::{BlockId, Module, Var};
...
pub fn decode_plv3_multifunc(bytecode: &[u8]) -> (Module, DecodeStats) {
    ...
    let mut block_terminated = false;
    let mut reader = Plv3Reader::new(bytecode);
    // Track spilled stack depth so successors can restore
    let mut spilled_depth: usize = 0;
    ...
    while !reader.at_end() {
        ...
        if needs_new_block {
            ...
            builder.switch_to(target_block);
            current_block = target_block;
            // Don't clear sym_stack — keep it continuous across the linear scan.
            // The PLV3 VM's stack persists across all blocks. Values from one
            // block's fall-through path carry into the next. At branch merge
            // points this may be inaccurate for the non-fall-through path,
            // but it's correct for the primary execution path.
            block_terminated = false;
        }
        ...
        // Trace stack depth at milestones
        if instruction_pc == 16560 || instruction_pc == 18487 || instruction_pc == 18587 {
            eprintln!("[stack-trace] PC {instruction_pc}: stack_depth={}", sym_stack.len());
        }
        ...
        match opcode_byte {
            ...
            147 => {
                let argc = reader.read_byte().unwrap_or(0) as usize;
                // Trace first 3 CALL instructions
                static CALL_DEC: std::sync::atomic::AtomicUsize = std::sync::atomic::AtomicUsize::new(0);
                let cd = CALL_DEC.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                if cd < 3 {
                    // Show what the callable variable is (the item just below the args)
                    let callable_idx = sym_stack.len().saturating_sub(argc + 1);
                    let callable_var = sym_stack.get(callable_idx);
                    eprintln!("[dec-call] #{cd}: PC {instruction_pc}, argc={argc}, stack_depth={}, callable_var={callable_var:?}", sym_stack.len());
                }
                let mut args: Vec<Var> = Vec::new();
                for _ in 0..argc { args.push(stack_pop(&mut sym_stack, &mut builder)); }
                args.reverse();
            ...
            // ═══ CONTROL FLOW ═══════════════════════════════════════
            189 | 20 | 42 | 169 | 89 | 197 | 116 | 195 => {

                match opcode_byte {
                    189 => { // JMP_FWD
                        let offset = reader.read_u16_be().unwrap_or(0) as usize;
                        let target = reader.position + offset;
                        let target_block = *block_map.entry(target).or_insert_with(|| builder.create_block(&format!("block_{target}")));
                        builder.jump(target_block);
                        stats.jumps += 1; block_terminated = true;
                    }
                    20 => { // JMP_BACK
                    ...
                    42 => { // JMP_IF_FALSY
                        let offset = reader.read_u16_be().unwrap_or(0) as usize;
                        let false_target = reader.position + offset;
                        let cond = stack_pop(&mut sym_stack, &mut builder);
                        let true_block = *block_map.entry(reader.position).or_insert_with(|| builder.create_block(&format!("block_{}", reader.position)));
                        let false_block = *block_map.entry(false_target).or_insert_with(|| builder.create_block(&format!("block_{false_target}")));
                        builder.branch_if(cond, true_block, false_block);
                        stats.branches += 1; block_terminated = true;
                    }
                    169 => { // JMP_FALSY_KEEP
                        let offset = reader.read_u16_be().unwrap_or(0) as usize;
                        let target = reader.position + offset;
                        let cond_var = sym_stack.last().copied().unwrap_or_else(|| stack_pop(&mut sym_stack, &mut builder));
                        let continue_block = *block_map.entry(reader.position).or_insert_with(|| builder.create_block(&format!("block_{}", reader.position)));
                        let target_block = *block_map.entry(target).or_insert_with(|| builder.create_block(&format!("block_{target}")));
                        builder.branch_if(cond_var, continue_block, target_block);
                        stats.branches += 1; block_terminated = true;
                    }
...
fn find_block_starts_in_region_skipping(
    ...
    while !reader.at_end() {
        ...
        match opcode {
            55 => {
                // MAKE_FUNC: consume header + following JMP_FWD.
                // The decoder treats this as a single unit — the JMP_FWD
                // should NOT create a block boundary.
                let _params = reader.read_byte().unwrap_or(0);
                let captures = reader.read_byte().unwrap_or(0);
                for _ in 0..captures { reader.read_byte(); }
                // Consume the JMP_FWD that follows
                let next_pc = base_pc + reader.position;
                if reader.position < region_bytecode.len()
                    && region_bytecode[reader.position] == 189
                {
                    reader.read_byte(); // consume 189
                    let offset = reader.read_u16_be().unwrap_or(0) as usize;
                    reader.position += offset; // skip to body_end
            ...
            189 => { // JMP_FWD (standalone, not after MAKE_FUNC)
                let offset = reader.read_u16_be().unwrap_or(0) as usize;
                let abs_pos = base_pc + reader.position;
            ...
            _ => { super::funcmap::skip_operands_pub(opcode, &mut reader); }
        }
    }
