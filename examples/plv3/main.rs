//! VM-Engine CLI — decode, analyze, and execute bytecode VMs.
//!
//! Run: cargo run
...
use vm_engine::ir::display::format_module;
use vm_engine::ir::validate;
use vm_engine::graph;
...
fn main() {
    ...
    // ═══ Direct Bytecode Interpreter Key Extraction ═══════════════
    println!("\n=== Key Extraction (DIRECT bytecode interpreter) ===");
    {
        let mut vm = interpreter::Plv3Vm::new(&bytecode);
        // Install web environment
        vm_engine::web::install_all(&mut vm.heap, vm.global, &vm_engine::web::WebConfig::default());

        struct DirectHook {
            btoa_captures: Vec<String>,
        }
        impl interpreter::InterpreterHook for DirectHook {
            fn on_native_call(&mut self, name: &str, args: &[vm_engine::value::Value], heap: &mut vm_engine::exec::heap::Heap) -> Option<vm_engine::value::Value> {
                match name {
                    "btoa" => {
                        let raw = args.first().cloned().unwrap_or(vm_engine::value::Value::Undefined);
                        eprintln!("[btoa-hook] raw arg: {:?}", &raw);
                        let input = match &raw {
                            vm_engine::value::Value::String(s) => s.clone(),
                            other => vm_engine::value::coerce::to_string(other),
                        };
                        self.btoa_captures.push(input.clone());
                        let encoded = vm_engine::web::encoding::base64_encode_raw(input.as_bytes());
                        Some(vm_engine::value::Value::string(encoded))
                    }
                    _ => None,
                }
            }
        ...
        let mut hook = DirectHook { btoa_captures: Vec::new() };
        match vm.run(&mut hook) {
            Ok(()) => println!("  completed: true"),
            Err(e) => println!("  stopped: {e}"),
        }
        println!("  instructions: {}", vm.instruction_count);
        println!("  stack depth:  {}", vm.stack.len());
        println!("  btoa calls:   {}", hook.btoa_captures.len());

        if !hook.btoa_captures.is_empty() {
            for (i, captured) in hook.btoa_captures.iter().enumerate() {
                println!("  btoa[{i}] len={}: {}", captured.len(), &captured[..captured.len().min(120)]);
                // Try to extract JSON keys
                if let Ok(parsed) = serde_json::from_str::<serde_json::Value>(captured) {
                    if let Some(obj) = parsed.as_object() {
                        let field_names = ["timestamp", "pathname", "clientWidth", "elapsed", "perf_now", "is_secure", "webdriver", "random"];
                        println!("  EXTRACTED KEYS ({}):", obj.len());
                        for (idx, (key, _)) in obj.iter().enumerate() {
                            let field = field_names.get(idx).unwrap_or(&"?");
                            println!("    {field:>15} → \"{key}\"");
                        }
                    }
                }
            ...
            println!("  [no btoa calls captured]");
        }
    }
